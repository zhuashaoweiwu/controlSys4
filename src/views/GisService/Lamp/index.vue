<template>
  <div class='map-wrap'>
    <el-amap vid='amapDemo' :zoom="zoom" :zooms="zooms" :center="center" :plugin='plugin'>
      ...
      <!--<el-amap-polyline v-for="(marker, index) in LightLine" :key="index + 'c'" :editable="marker.editable"-->
      <!--:path="marker.path" :events="marker.events"-->
      <!--:strokeColor="marker.state === 1 ? '#17BEB0' : '#FF2851'"></el-amap-polyline>-->
    </el-amap>
    <!-- 搜索栏 -->
        ...
        <div class="form-wrap">
          <el-form :model="addScene" :rules="addScenerRules" ref="addScene" label-width="100px">
            ...
            <el-form-item label="场景描述" prop="desc">
              <el-input type="textarea" v-model="addScene.desc" placeholder="请输入" style="width:500px"
                        size='small'></el-input>
            </el-form-item>
          </el-form>
        </div>
            ...
            <div class="transfer-wrap clearfix">
              <div class="select-wrap">
                ...
                <el-table
                  ...
                  <el-table-column label="UID" width="40" prop="id"></el-table-column>
                  <el-table-column label="灯杆" width="100" prop="lamppost"></el-table-column>
                  <el-table-column label="灯头号" prop="lamphead" width="100"></el-table-column>
...
<script>
  ...
  export default {
    ...
    created () {
      let projectQuery = this.$router.history.current.query
      this.projectId = projectQuery.projectId
      // 获取所有项目下面的控制柜
      listElebox(this.projectId).then(res => {
        this.EleboxList = res.data ? res.data : this.EleboxList
        // 获取到所有控制柜之后生成每个控制柜的连线
        // this.generalLightLine(this.projectId, this.EleboxList)
        // 获取项目下面所有灯具
        listLighting(this.projectId).then(res => {
          this.lightList = res.data
          this.generalLightMarks(this.lightList)
          // that.lampsDialog = true
        }).catch(() => {
        })
        this.setCenter(res.data[0].longitude, res.data[0].latitude)
        this.generalEleboxMarks(this.EleboxList)
      }).catch(() => {
      })
    },
    destroyed () {
